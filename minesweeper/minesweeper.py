...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        if new_sentence not in self.knowledge and len(neighbour_cells) > 0:
            ...
            for sentence in self.knowledge:
                if len(sentence.known_mines()) > 0 and not sentence.known_mines().issubset(self.mines):
                    for cell in sentence.known_mines():
                        known_m.add(cell)

                if len(sentence.known_safes()) > 0 and not sentence.known_safes().issubset(self.mines):
